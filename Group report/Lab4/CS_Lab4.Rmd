...
$$
\begin{aligned}
p(\mu_{1}|\vec\mu_{-1},\vec Y)
\propto~&\exp \Big(-\frac{\sum_{i=2}^{n}(y_i-\mu_i)^2+\sum_{i=2}^{n}(\mu_{i}-\mu_{i-1})^2}{0.4})~~,
\end{aligned}
$$
and
$$
\begin{aligned}
p(\mu_{n}|\vec\mu_{-n},\vec Y)
\propto&~~ \exp \Big(-\frac{\sum_{i=1}^{n-1}(y_i-\mu_i)^2+\sum_{i=2}^{n-1}(\mu_{i}-\mu_{i-1})^2}{0.4})~~.
\end{aligned}
$$
Since
...
\propto&\frac{\exp \Big(-\frac{\sum_{i=1}^{n}(y_i-\mu_i)^2+\sum_{i=2}^{n}(\mu_{i}-\mu_{i-1})^2}{0.4})}{\exp \Big(-\frac{\sum_{k=1,k\neq i}^{n-1}(y_k-\mu_k)^2+\sum_{k=2,k\neq i}^{n-1}(\mu_{k}-\mu_{k-1})^2}{0.4})}\\
\propto&~~\exp(-\frac{(y_i-\mu_i)^2+(\mu_i-\mu_{i-1})^2}{2\sigma^2})\\
\propto&~~ \exp(-\frac{(\mu_i-(y_i+\mu_{i-1})/{2})^2}{2\sigma^2/2})\\
=&~~ \exp(-\frac{(\mu_i-(y_i+\mu_{i-1})/{2})^2}{\sigma^2}) ~~.
\end{aligned}
$$
The deduction shows that 
$$(\mu_i|\vec\mu_{-i},\vec{Y})\sim N(\frac{(y_i+\mu_{i-1})}{2},\frac{\sigma^2}{2}).$$


## 4.Use the distributions derived in Step 3 to implement a Gibbs sampler 

that uses $\vec \mu_0=(0,...,0)$ as a starting point. Run the Gibbs sampler to obtain 1000 values of $\vec \mu$ and then compute the expected value of $\vec \mu$ by using a Monte Carlo approach. Plot the expected value of $\vec \mu$ versus X and Y versus X in the same graph. Does it seem that you have managed to remove the noise? Does it seem that the expected value of $\vec \mu$ can catch the true underlying dependence between Y and X?

```{r}
n = 10000
X_0 = rep(0, n)
gibbs = function(n, x_0){

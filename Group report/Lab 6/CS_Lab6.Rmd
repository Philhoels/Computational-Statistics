...
$$
\begin{aligned}
\mathcal{L}(\lambda|Y,Z)=&\prod(\frac{X_i}{\lambda}e^{-\frac{X_i}{\lambda}Y_i})~\cdot~ \prod(\frac{ X_i}{2\lambda}e^{-\frac{X_i}{2\lambda}Z_i})\\\\
...
$$ \mathcal l(\lambda|Y,Z)=\prod (\ln X_i)-n\ln\lambda-\frac{\sum X_iY_i}{\lambda}+\prod(\ln X_i)-n\ln(2\lambda)-\frac{1}{2\lambda}\sum X_i Z_i.$$
...
$$
\begin{aligned}
Q(\lambda,\lambda_k)=&E[\mathcal{l}(\lambda|Y,Z)|\lambda_k,Y,Z]\\\\
...
### maximization 
To obtain the maximum likelihood estimate of $\lambda$, we need to solve that the partial erivative equals 0.
$$
\begin{aligned}
...
ggplot(data = data24) +
  ...
  ggtitle("Z & Y versus X") + 
  ylab("Z & Y")+
  scale_color_discrete(labels=c("Y","Z","E[Y]","E[Z]"))
